...
# Example usage:
csv_file_path = 'spinny_bangalore.csv'  # Replace with the actual path to your CSV file
...
def process_and_write_to_new_file(input_file_path, output_file_path):
    ...
    df['CarName'] = df['CarName'].apply(lambda x: ' '.join(str(x).split()[1:3]) if len(str(x).split()) >= 3 else x)
...
# Example usage:
input_file_path = 'spinny_bangalore.csv'  # Replace with the actual path to your input CSV file
output_file_path = 'separated_csvs/spinny_bangalore_separated.csv'  # Replace with the desired output file path
